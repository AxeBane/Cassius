...
let commands = {
	...
	hotd: function (target, room, user) {
		...
		if (database.hotd) {
			if (!typo && Date.now() - database.hotd.time < 61200000) return this.say(text + "Sorry, but at least 17 hours must have passed since the HOTD was last set in order to set it again!");
		}
		let hasPerms = false;
		if (database.scribeShop) {
			if (typo || (!(room instanceof Users.User) && user.hasRank(room, '+'))) {
	...
	botd: function (target, room, user) {
		...
		if (targets.length < 3) return this.say(text + "Invalid arguments specified. The format is: __book__, __genre__, __author__, __summary__.");
		let botd = {
			book: targets[0].trim(),
			genre: targets[1],
			author: targets[2],
			summary: targets[3].trim(),
		};
		if (!typo) {
			botd.time = Date.now();
			botd.user = user.name;
